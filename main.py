...
for loc in airport_loc:
    exclude_lst = ['YATN', 'YATN', 'YUTA', 'YBSM', 'YBRB', 'YCLN', 'YPFT', 'YLBD', 'YUPA', 'YXGS', 'YIGM', 'YXLG',
                   'YXMA', 'YXMW', 'YXMO', 'YXFV', 'YPAM', 'YUPE', 'YSCF', 'YSBY', 'YTES', 'YTDN',
        ...
        def parse_obj(lt_objs):
            for obj in lt_objs:
                if isinstance(obj, pdfminer.layout.LTTextBoxHorizontal):
                    if round(obj.bbox[1]) >= 45 and round(obj.bbox[1]) <= 60:
                        if obj.bbox[0] < 300:
                            if loc == "(YWWI)":
